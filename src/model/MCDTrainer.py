...
class MCDTrainer(RhinoTrainer):
    ...
    def compute_loss_terms(self,
        ...
        graph_selection_prior_term = ce_loss(W.unsqueeze(1).repeat(1, batch).transpose(
            0, 1), self.mixture_selection.get_probs(sample_idx).transpose(0, 1))/(self.num_fragments)*self.total_samples
        # graph_selection_prior_term /= batch

        # ********** graph selection logits entropy ***

        graph_selection_entropy_term = - \
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.use_true_graph:
            pred_mixture_idx = self.mixture_selection.get_mixture_indices(
    ...
    def get_cluster_indices(self):
        ...
        pred_cluster_idx = self.mixture_selection.get_mixture_indices(
            torch.arange(self.total_samples))

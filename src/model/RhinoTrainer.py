...
from src.utils.metrics_utils import compute_shd
...
class RhinoTrainer(BaseTrainer):
    ...
    def __init__(self,
        ...
        assert likelihood_loss in ['mse', 'flow']
        ...
        if self.training_procedure == 'auglag':
            ...
            if self.skip_auglag_epochs > 0:
                print(
                    f"Not performing augmented lagrangian optimization for the first {self.skip_auglag_epochs} epochs...")
    ...
    def training_step(self, batch, batch_idx):
        ...
        X_history, x_current, _, idx, _ = batch
    ...
    def validation_func(self, X_history, x_current, adj_matrix, G, idx):
        ...
        if self.aggregated_graph:
            ...
            f1_inst = 2*np.sum(tp[:, 0]) / (2*np.sum(tp[:, 0]) +
                                            np.sum(fp[:, 0]) + np.sum(fn[:, 0]))
            f1_lag = 2*np.sum(tp[:, 1:]) / (2*np.sum(tp[:, 1:]) +
                                            np.sum(fp[:, 1:]) + np.sum(fn[:, 1:]))
    ...
    def validation_step(self, batch, batch_idx):
        ...
        loss_terms = self.validation_func(
            X_history, x_current, adj_matrix, expanded_G, idx)
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        probs = self.adj_matrix.get_adj_matrix(
            do_round=False).unsqueeze(0).repeat(batch_size, 1, 1, 1)

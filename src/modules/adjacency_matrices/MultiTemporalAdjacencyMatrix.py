...
class MultiTemporalAdjacencyMatrix(pl.LightningModule, AdjMatrix):
    def __init__(
        ...
        if self.threeway:
            ...
            self.lower_idxs = torch.unbind(
                torch.tril_indices(self.num_nodes, self.num_nodes, offset=-1, device=self.device), 0
            )
        else:
            self.logits_inst = nn.Parameter(
    ...
    def _triangular_vec_to_matrix(self, vec):
        ...
        """
        N = vec.shape[1]
        output = torch.zeros((N, self.num_nodes, self.num_nodes), device=self.device)
    ...
    def entropy(self) -> torch.Tensor:
        ...
        if not self.disable_inst:
            if self.threeway:
                dist = td.Categorical(logits=self.logits_inst[:, :].transpose(0, -1))
                entropies_inst = dist.entropy().sum()
            else:
                dist = td.Categorical(logits=self.logits_inst[1, ...] - self.logits_inst[0, ...])
                I = torch.arange(self.num_nodes)
                dist_diag = td.Categorical(logits=self.logits_inst[1, :, I, I] - self.logits_inst[0, :, I, I])
        ...
        dist_lag = td.Independent(td.Bernoulli(logits=self.logits_lag[1, :] - self.logits_lag[0, :]), 3)
    ...
    def sample_A(self) -> torch.Tensor:
        ...
        if not self.disable_inst:
            if self.threeway:
                ...
                sample = F.gumbel_softmax(self.logits_inst, tau=self.tau_gumbel, hard=True, dim=0)[1, ...]

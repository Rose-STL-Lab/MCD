...
def adjacency_f1(adj_matrix, predictions):
    ...
    adj = np.concatenate((adj_diag, np.logical_or(adj_lower, adj_upper).flatten().astype(int)))
    
    pred_diag = np.diagonal(predictions, axis1=-2, axis2=-1).flatten()
    pred_lower = predictions[..., L[0], L[1]]
    pred_upper = predictions[..., U[0], U[1]]
    pred = np.concatenate((pred_diag, np.logical_or(pred_lower, pred_upper).flatten().astype(int)))
...
def calculate_expected_shd(scores, adj_matrix, aggregated_graph=False, n_trials=100):
    ...
    for i in range(n_trials):
        ...
        if aggregated_graph:
            shd = compute_shd(adj_matrix, draw, aggregated_graph=aggregated_graph)
        else:
            shd, shd_lag, shd_inst = compute_shd(adj_matrix, draw, aggregated_graph=aggregated_graph)
        totals_shd += shd
...
def evaluate_results(scores, 
    ...
    eshd = calculate_expected_shd(np.abs(scores/(np.max(scores)+1e-4)), adj_matrix, aggregated_graph)
    ...
    if pred_cluster_indices is not None and true_cluster_indices is not None:
        ...
        _, true_cluster_indices = np.unique(adj_matrix, return_inverse=True, axis=0)
        _, pred_cluster_indices = np.unique(predictions, return_inverse=True, axis=0)
        metrics['cluster_acc'] = cluster_accuracy(true_idx=true_cluster_indices, 
                                                  pred_idx=pred_cluster_indices)

...
def process_netsim(args):
    ...
    for i in simulations:
        ...
        for j in range(X.shape[0]):
            ...
            X_dict_by_matrix[(adj_matrix[j].tobytes(),
                              X.shape[1])].append(X[j])

        if N_nodes == 15 and timepoints == 200:
            print("SIMULATION", i)
    for N_nodes, timepoints in N_t_dict:
        X = np.concatenate(N_t_dict[(N_nodes, timepoints)]['X'], axis=0)
        adj_matrix = np.concatenate(
            N_t_dict[(N_nodes, timepoints)]['adj_matrix'], axis=0)
        np.savez(os.path.join(
            args.save_dir, f'netsim_{N_nodes}_{timepoints}.npz'), X=X, adj_matrix=adj_matrix)
    ...
    for key, T in X_dict_by_matrix:
        ...
        if not os.path.exists(os.path.join(args.save_dir, 'grouped_by_matrix', f'{N}_{T}')):
            os.makedirs(os.path.join(args.save_dir,
                        'grouped_by_matrix', f'{N}_{T}'))
        X = np.array(X_dict_by_matrix[(key, T)])
        np.savez(os.path.join(args.save_dir, 'grouped_by_matrix',
                 f'{N}_{T}', f'netsim_{counts[(N, T)]}.npz'), X=X, adj_matrix=adj_matrix)
        counts[(N, T)] += 1
    ...
    for N_nodes in [15, 50]:
        timepoints = 200
        num_graphs = 3
        permutation_pool = [np.random.permutation(
            np.arange(N_nodes)) for i in range(num_graphs)]
        ...
        np.savez(os.path.join(
            args.save_dir, f'netsim_{N_nodes}_{timepoints}_permuted.npz'), X=X, adj_matrix=adj_matrix)

...
def generate_stock(args):
    ...
    for sector in df['Sector'].unique():
        ...
        np.save(os.path.join(args.save_dir,
                f'X_{sector.replace(" ", "")}.npy'), X_sector)

    with open(os.path.join(args.save_dir, 'dates.csv'), 'w', encoding="utf-8") as f:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        "Stock data generator from Yahoo Financials")
    parser.add_argument("--start_date", type=str, default='2016-01-01')
    parser.add_argument("--end_date", type=str, default='2023-07-01')
    parser.add_argument('--chunk_size', type=int, default=31,
                        help='Number of days to chunk together into one sample. Default is 31 days')

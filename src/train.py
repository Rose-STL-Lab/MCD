...
def run(cfg: DictConfig):
    ...
    if dataset_path in cfg:
        ...
        raise Exception("No model found in the config. Try running with option: python3 -m src.train +dataset=<dataset> +<dataset>=<model>")
...
def train(cfg):
    ...
    X, adj_matrix, aggregated_graph, lag, data_dim = load_data(cfg.dataset, cfg.dataset_dir, cfg)
    ...
    if 'use_indices' in cfg:
        f_path = os.path.join(cfg.dataset_dir, cfg.dataset, f'{cfg.use_indices}_seed={cfg.random_seed}.npy')
        mix_idx = torch.Tensor(np.load(f_path))
        model.set_mixture_indices(mix_idx)


    training_needed = cfg.model == 'rhino' or cfg.model == 'mcd'
    ...
    if training_needed:
        monitor_checkpoint_based_on = cfg.monitor_checkpoint_based_on # either val_loss or likelihood
        ckpt_choice = 'best'
    ...
    if training_needed and ckpt_choice == 'best':
        if model_name == 'mcd':
            np.save(os.path.join('results', dataset, f'{model_name}_{checkpoint_callback.best_model_score.item()}_k{cfg.trainer.num_graphs}.npy'), scores)    
        else:
            np.save(os.path.join('results', dataset, f'{model_name}_{checkpoint_callback.best_model_score.item()}.npy'), scores)
    else:
    ...
    if model_name == 'pcmci' or model_name == 'dynotears':
        metrics['model'] += "_singlegraph_" + str(cfg.trainer.single_graph) + "_grouped_" + str(cfg.trainer.group_by_graph)
    if model_name == 'mcd':
        metrics['model'] += "_trueindex_" + str(cfg.trainer.use_correct_mixture_index)
        if 'use_indices' in cfg:
            metrics['model'] += '_' + cfg.use_indices
    if (model_name == 'rhino' or model_name == 'mcd') and 'linear' in cfg.decoder and cfg.decoder.linear:
